...
# Dados validados do estudo S001
novo_estudo = [
    "S001",
    ...
    "Equipas de suporte de repositórios (NAHDAP/CEDAR) e equipa técnica de desenvolvimento da DMPTool",
    "Projeto financiado / Obrigatório",
    "Submissão da proposta / início do projeto (documento vivo)",
    "DMPTool",
    "HEAL Platform; CEDAR; NAHDAP",
    "Modelo estruturado com exemplos (sample responses) e orientações disciplinares específicas",
    ...
    "Complexidade de dados multiescala e heterogéneos; salvaguarda ética de coortes de dor/adição; falta prévia de diretrizes",
    "não relatado",
...
def atualizar_ou_inserir_estudo():
    ...
    # Ler o ficheiro existente se ele existir
    if csv_path.exists():
        with open(csv_path, mode="r", encoding="utf-8") as f:
            leitor = csv.reader(f)
            for linha in leitor:
                if linha and linha[0] == novo_estudo[0]:
                    linhas.append(novo_estudo) # Substitui com os dados novos
                    atualizado = True
                else:
                    linhas.append(linha)
    ...
    # Escrever de volta para o CSV
    with open(csv_path, mode="w", encoding="utf-8", newline="") as f:
        escritor = csv.writer(f)
        escritor.writerows(linhas)
        
    print(f"Sucesso! Estudo {novo_estudo[0]} atualizado/inserido na base de dados com 30 campos.")
